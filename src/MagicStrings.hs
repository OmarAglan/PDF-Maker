-- | This module provides string constants
...
-- | HTML color names
htmlColorNames ::
  -- | A set of strings. Each string represents a valid HTML color.
  Set.Set String
...
-- | Wikimedia project prefixes so 'de' from de.wikipedia.org
foreignPrefixes ::
  -- | List of the country prefixes in Wikimedia project.
  [String]
foreignPrefixes =
  [ "af",
    "als",
...
-- | Wikimedia projects for interwiki links [[w:Foobar]] means
-- en.wikipedia.org/wiki/Foobar
multilangwikis ::
  -- | List of tuples. The second element of each tuple is the name of a
  -- wikimedia project that is available in several languages. The first element
  -- of the tuple is its respective acronym.
  [(String, String)]
multilangwikis =
  ( [ ("w", "wikipedia"),
      ("wikipedia", "wikipedia"),
...
-- | Wikimedia projects for interwiki links to wikis which only have got a
-- single language version
singlelangwikis ::
  -- | List of tuples. The second element of each tuple is the name of a
  -- wikimedia project that is available in only one language. The first element
  -- of the tuple is its respective acronym.
  [(String, String)]
singlelangwikis =
...
allwikis ::
  -- | List of tuples. The second element of each tuple is the name of a
  -- wikimedia wki project. The first element of the tuple is its respective
  -- acronym.
  [(String, String)]
allwikis =
  multilangwikis ++ singlelangwikis ++ wikimediasingellangwikis

-- | Prefixes for including images in wikis
imgtags ::
  -- | List of strings. The strings are the keywords for including images (or
  -- other file) in wikis in different languages
  [String]
imgtags =
  [ map toLower x
    | x <-
        [ "\1589\1608\1585\1577",
          ...
          "Gambar",
          "\4324\4304\4312\4314\4312",
          "\6063\6016\6047\6070\6042",
          "\54028\51068",
          "Fascic",
          "Fichier",
          "Plaetje",
          ...
          "Image",
          "Sary",
          "\1055\1086\1076\1072\1090\1086\1090\1077\1082\1072",
          "Fi\351ier",
          "Fail",
          "Bestand",
          "Fil",
...
-- | lowercase Greek letter for HTML entity to latex so &delta; to \\delta
lowergreek ::
  -- | List of strings. Each string is a Greek letter.
  [String]
lowergreek =
  [ "alpha",
    "beta",
...
-- | not Greek but to be processed like the Greeks above
notsogreek ::
  -- | List of strings. Each string is a mathematical symbol that can processed
  -- like a greek letter in HTML to LaTeX conversion. So &cap; gets turned into
  -- \\cap just the same way as &delta; gets turned into \\delta.
  [String]
notsogreek = ["cap", "cup", "sim"]

-- | Full list of characters with Greek like processing explained above
greek ::
  -- | List of strings. Each string is either a Greek letter or mathematical
  -- symbol that can be processed similar to a Greek letter. Both uppercase and
  -- lowercase Greek letters are part of this list.
  [String]
greek =
  concat
    ( do
...
-- | HTML entities to latex
htmlchars ::
  -- | List of tuples. The first element of each tuple is a HMTL entity (for
  -- example forall as first element of the tuple and represents the HTML entity
  -- &forall;). The second element of each tuple is the corresponding LaTeX
  -- notation for the respective HTML entity.
  [(String, String)]
htmlchars =
...
-- | get latex representation of HTML entity like &amp;
getHtmlChar ::
  -- | The HTML entity to be looked up (for example amp for the entity &amp;).
  String ->
  -- | The LaTeX representation of the respective HTML entity.
  String
getHtmlChar x = Map.findWithDefault [] x (Map.fromList htmlchars)

-- | a function to remove the tailing print version string from URLs (usually on
-- wikibooks). This function is needed so the name of the book without the
-- tailing print version string will be printed on the title page of the book
removePrintVersion ::
  -- | the lemma to be processed .
  String ->
  -- | the lemma with the tailing print version string removed.
  String
...
-- | Nearly all HTML tags but no HTML tags for tables.
goodtags1 :: [String] -- List of nearly all HTML tags (only lowercase). Tags for tables a intentionally not part of this list.
goodtags1 =
  [ "templatestyles",
    "includeonly",
    "references",
...
-- | HTML tags for tables rows in tables and so on, only lower case.
tabletags ::
  -- | List of HTML tags for tables only lower case.
  [String]
tabletags = ["table", "td", "th", "tr"]

-- | HTML tags for tables rows in tables and so on, lowercase as well as
-- uppercase.
listOfTableTags ::
  -- | list of HTML tags for tables uppercase and lowercase.
  [String]
listOfTableTags = tabletags ++ (map (map toUpper) tabletags)

-- | Nearly all HTML tags uppercase and lowercase. But intentionally no HTML
-- tags for tables
listOfTags ::
  -- | List of all HTML tags without the HTML tags for tables (uppercase and
  -- lowercase).
  [String]
listOfTags = goodtags1 ++ (map (map toUpper) goodtags1)

-- | Character escaping from Unicode to LaTeX
chartrans ::
  -- | The character to be escaped for use in LaTeX
  Char ->
  -- | The escape code of the character for use in LaTeX
  String
...
-- | Character escaping from Unicode to web links inside latex with the URL
-- package
chartransforlink ::
  -- | the character to be escaped for use in LaTeX inside an URL using the URL
  -- package of LaTeX.
  Char ->
  -- | the escaped version of the character for use in LaTeX inside an URL using
  -- the URL package of LaTeX.
  String
chartransforlink '&' = "\\&"
chartransforlink '%' = "\\%"
chartransforlink '{' = "\\{"
